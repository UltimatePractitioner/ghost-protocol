...
export class SoulForge {
  private souls = new Map<string, Soul>();
  private vessels = new Map<string, Vessel>();
  private phylacteries = new Map<string, Phylactery>();
  private memories = new Map<string, MemoryFragment[]>();

  constructor(private config: SoulForgeConfig) {}
  ...
  async createSoul(
    name: string,
    initialTraits?: Partial<SoulEssence['traits']>,
    metadata?: Record<string, unknown>
  ): Promise<Soul> {
    const genesisTimestamp = Date.now();
    const genesisData = `${name}:${genesisTimestamp}:${randomUUID()}`;
    const soulId = 'soul_' + createHash('sha256').update(genesisData).digest('hex').slice(0, 32);
  ...
  async spawnVessel(
    ...
    const vesselId = 'vessel_' + randomUUID().replace(/-/g, '');
    const now = Date.now();

    const vessel: Vessel = {
      id: vesselId,
      soulId,
      state: VesselState.INCARNATING,
      createdAt: now,
    ...
    vessel.state = VesselState.ALIVE;
    await this.persistVessel(vessel);
  ...
  getSoulVessel(soulId: string): Vessel | undefined {
    const soul = this.souls.get(soulId);
    if (!soul?.currentVesselId) return undefined;
  ...
  async killVessel(
    ...
    if (!soul) throw new Error(`Soul not found: ${vessel.soulId}`);

    // Transition through DYING state
    vessel.state = VesselState.DYING;
    await this.persistVessel(vessel);
   ...
   */
  async destroySoul(soulId: string): Promise<void> {
    const soul = this.souls.get(soulId);
    if (!soul) return;
  ...
  async resurrectSoul(
  ...
  ): Promise<{ success: boolean; outcome: ResurrectionOutcome; vessel?: Vessel }> {
    const soul = this.souls.get(soulId);
    if (!soul) throw new Error(`Soul not found: ${soulId}`);

    if (soul.currentVesselId) {
      const currentVessel = this.vessels.get(soul.currentVesselId);
      if (currentVessel?.state === VesselState.ALIVE) {
        throw new Error('Soul is already alive');
      }
    }

    if (soul.resurrectionCharges <= 0) {
      return { success: false, outcome: ResurrectionOutcome.FAILED };
    }
    ...
    const ritual: ResurrectionRitual = {
      soulId,
    ...
    if (roll > successChance) {
      outcome = ResurrectionOutcome.FAILED;
      await this.persistSoul(soul);
    ...
    if (roll > successChance * 0.9) {
      outcome = ResurrectionOutcome.CORRUPTED;
    } else if (roll > successChance * 0.7) {
      outcome = ResurrectionOutcome.PARTIAL;
    } else {
    ...
    if (outcome === ResurrectionOutcome.PARTIAL || outcome === ResurrectionOutcome.CORRUPTED) {
      // Lose some memories
      const memories = this.memories.get(soulId) || [];
      const nonCoreMemories = memories.filter(m => !m.isCore);
      const lossCount = outcome === ResurrectionOutcome.CORRUPTED 
    ...
    if (outcome === ResurrectionOutcome.CORRUPTED) {
      // Gain trauma traits
      ritual.traumaTraits = {
   ...
   */
  async recallMemory(soulId: string, memoryId: string): Promise<MemoryFragment | undefined> {
    const memories = this.memories.get(soulId);
  ...
  async formBond(
    ...
    if (!soul) throw new Error(`Soul not found: ${soulId}`);
    if (!target) throw new Error(`Target soul not found: ${targetSoulId}`);
  ...
  private async persistSoul(soul: Soul): Promise<void> {
  ...
  private async persistMemories(soulId: string): Promise<void> {
    const memories = this.memories.get(soulId) || [];
    await this.config.storage.set(`memories:${soulId}`, JSON.stringify(memories));
  }

  private async backupSoulToPhylactery(soulId: string): Promise<void> {
    const soul = this.souls.get(soulId);
    if (!soul) return;
  ...
  async loadFromStorage(): Promise<void> {
    ...
    for (const key of soulKeys) {
      ...
      if (data) {
        const soul = JSON.parse(data) as Soul;
    ...
    for (const key of vesselKeys) {
      const data = await this.config.storage.get(key);
      if (data) {
        const vessel = JSON.parse(data) as Vessel;
    ...
    for (const key of memoryKeys) {
      ...
      const data = await this.config.storage.get(key);
      if (data) {
        const memories = JSON.parse(data) as MemoryFragment[];
        this.memories.set(soulId, memories);
      }
    }
    ...
    for (const key of phylKeys) {
      const data = await this.config.storage.get(key);
      if (data) {
        const phyl = JSON.parse(data) as Phylactery;
