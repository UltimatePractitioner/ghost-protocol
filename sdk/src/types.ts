...
export enum ResurrectionOutcome {
  SUCCESS = 'success',
  PARTIAL = 'partial',
  FAILED = 'failed',
  CORRUPTED = 'corrupted',
...
export type GhostEvent =
  | { type: 'SOUL_CREATED'; soul: Soul }
  | { type: 'VESSEL_SPAWNED'; vessel: Vessel }
  | { type: 'SOUL_BOUND'; soulId: string; vesselId: string }
  | { type: 'HEARTBEAT'; pulse: Pulse }
  | { type: 'VITALITY_LOW'; vesselId: string; vitality: number }
  | { type: 'DEATH'; vesselId: string; soulId: string; reason: DeathReason }
  | { type: 'RESURRECTION_STARTED'; ritual: ResurrectionRitual }
  | { type: 'RESURRECTION_COMPLETED'; soulId: string; outcome: ResurrectionOutcome }
  | { type: 'MEMORY_FORMED'; soulId: string; memory: MemoryFragment }
  | { type: 'MEMORY_DECAYED'; soulId: string; memoryId: string }
  | { type: 'BOND_FORMED'; bond: SoulBond }
  | { type: 'KARMA_CHANGED'; soulId: string; delta: number; newKarma: number };
